...
bool Collider::isCollidingWith(const Collider* other)
{
    if (_type == ColliderType::COLLIDER_CIRCLE && other->_type == ColliderType::COLLIDER_CIRCLE) {
        glm::vec2 p1 = _parent->getPosition() + _offset;
        glm::vec2 p2 = other->_parent->getPosition() + other->_offset;
...
{
#ifdef DEBUG_MODE
    game.renderFillCircle(_parent->getRenderPosition() + _offset, _size);
#endif
}